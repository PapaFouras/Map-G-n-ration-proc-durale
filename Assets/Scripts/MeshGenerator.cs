...
{
    public static MeshData GenerateTerrainMesh(float[,] borderedSizeMap, float borderedSizeMultiplier, AnimationCurve _borderedSizeCurve, int levelOfDetail, bool useFlatShading){
        ...
        for (var y = 0; y < borderedSize; y+= meshSimplificationIncrement)
        {
            for (var x = 0; x < borderedSize; x+=meshSimplificationIncrement)
            {
                ...
                float height = borderedSizeCurve.Evaluate(borderedSizeMap[x,y]) * borderedSizeMultiplier;
